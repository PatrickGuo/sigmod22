...
enum {
  GUID_INVALID = 0,
  GUID_INPUT = 10, // decor: this is marked with src input tensor's associated op.guid
  GUID_WEIGHT = 11, // decor: weight matrix, batchNorm scale/bias/...
  GUID_PRESERVED = 19,
};
...
enum OpType {
  ...
  OP_FUSE_CONV_BATCHNORM, //https://tehnokv.com/posts/fusing-batchnorm-and-conv/
};
...
struct SplitInfo {
  ...
  int num; // decor: how many splits.
  int pos[MAX_NUM_SPLITS]; // decor: e.g., {0, 2, 5} means [0,2), [2, 5), [5, end)
};
...
public:
  ...
  OpType type;
  float runtime;
  float difference = 1; // DeCor : output difference.
};

// Graph holds the connectivity info between ops, and is the outmost interface.
// Model is op factory (gen op, alloc mem, ...), and holds globally shared cuda res/handlers
class Graph {
public:
  Graph();
  ...
  // helper func for DeCor decor
  float calculate(float indiff);
private:
...
public:
  ...
  void collect_costs(float& exe_time, float& flops, float& mem_acc, int& num_kernels);
  void diff(); // DeCor : diff esti. kernel
#ifdef USE_CUDNN
...
public:
  ...
  void unmap(void);
  void collect_costs(float& exe_time, float& flops, float& mem_acc, int& num_kernels);
  // DeCor : use default calcdiff behavior
public:
#ifdef USE_CUDNN
...
// decor This operator simply fuses the conv weights and batchnorm together. 
// -- Input: conv_weight, Output: batch normalized conv_weight.
// -- The output TENSOR t0 of this operator will be used as: matmal(im2col'ed_input, t0).
// -- Therefore, it does not change input, and hence no diff() function involved.
...
// same as fuseconvbatchnorm no diff() effect.
...
public:
  ...
  cudnnHandle_t dnn;
  cublasHandle_t blas;
  cusolverDnHandle_t solver; // DeCor : svd
  cudnnTensorDescriptor_t inputTensor, biasTensor, outputTensor;
  cudnnFilterDescriptor_t filterDesc;
