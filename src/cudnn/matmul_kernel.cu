...
void Matmul::forward(bool block)
{
  ...
  int m = inputs[0].dim[numDim-2]; // decor inputs[0]: m*k; inputs[1]: k*n => outputs : m*n
  int n = inputs[1].dim[numDim-1];
  int k = inputs[0].dim[numDim-1];
  cublasOperation_t transA, transB;
  int lda, ldb, ldc;
  if (inputs[0].stride[numDim-2] == 1) { // decor: row's stride=1 means column-first storage format.
    transA = CUBLAS_OP_N;
