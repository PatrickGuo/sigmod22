...
#include "dnndiff/substitution.h"
#include <iostream>
#include <fstream>
using namespace std;
...
float Graph::run(void)
{
  ...
  // decor: this will initialize the in degrees of each op. the SrcOps (ready ops) will be added to opList.
  for (it = inEdges.begin(); it != inEdges.end(); it++) {
    int cnt = 0;
    std::set<Edge, EdgeCompare> inList = it->second;
    std::set<Edge, EdgeCompare>::const_iterator it2;
    for (it2 = inList.begin(); it2 != inList.end(); it2++) {
      if (it2->srcOp.guid > GUID_PRESERVED) cnt ++; // GUID_PRESERVED masks the GUID_INPUT, GUID_WEIGHT, ... special tensors.
    }
    todos[it->first] = cnt;
    if (todos[it->first] == 0)
      opList.push_back(it->first);
  ...
  size_t i = 0;
  // decor: opList contains the current ready ops
  while (i < opList.size()) {
    Op op = opList[i++];
    std::set<Edge, EdgeCompare> outList = outEdges[op];
    std::set<Edge, EdgeCompare> inList = inEdges[op];
...
// decor DeCor calculate static difference (following topo order) between models given the diff graph.
float Graph::calculate(float indiff) {
  float outdiff = 1.0;
  std::map<Op, int, OpCompare> todos;
  std::map<Op, std::set<Edge, EdgeCompare>, OpCompare>::const_iterator it;
  std::vector<Op> opList;
  ...
  while (i < opList.size()) {
    ...
    std::set<Edge, EdgeCompare>::const_iterator it2;
    assert(inList.size() > 0);
    // visit the current op, calculate the diff
    op.ptr->diff();
    final_diff = op.ptr->difference;
    for (it2 = outList.begin(); it2 != outList.end(); it2++) {
      todos[it2->dstOp] --;
      //printf("myOp(%zu) dstOp(%zu) dstType(%d) dstTodos(%d)\n",
      //    it2->srcOp.guid, it2->dstOp.guid,
